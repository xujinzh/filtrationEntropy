...
def ComputeEntropy(x, M):
    ...
    h = [0] * (len(dm) + 1)  # 初始化数据熵向量
    hdm = [0] * (len(dm) + 1)  # 初始化用于画数据熵图的数据熵向量
            ...
            for j in np.arange(len(x)):  # 内层两个for循环用于计算圆邻域内的点数
                for t in np.arange(j + 1, len(x)):
                    if DM[j][t] <= dm[k - 1]:
                        circle[j] = circle[j] + 1
                        circle[t] = circle[t] + 1
            h[k] = h[k] - (circle[i] / sum(circle)) * math.log(circle[i] / sum(circle))  # 计算原始数据熵，归一化之前的值
            if i == len(circle) - 1:
                print("NumbersOfPointsInCircle: {0} when distance is {1}".format(circle, dm[k - 1]))  # 打印圆邻域向量
            circle = [1] * len(x)  # normalized 再次进行归一化圆邻域向量
        e2 = np.arange(dm[k - 1], dm[k], 0.00001)  # 离散化区间（dm[k - 1], dm[k]）用以画图
        h[k] = h[k] / math.log(len(x))  # 归一化数据熵
        hdm[k] = h[k] * np.ones(len(e2))  # 离散化数据熵用以画图
        plt.plot(e2, hdm[k], color = "black")  # 画出距离为dm[k - 1]->dm[k]的数据熵图

    e1 = np.arange(0, dm[0], 0.00001)
    h[0] = (math.log(len(x)) / math.log(len(x)))
    hdm[0] = h[0] * np.ones(len(e1))
    plt.plot(e1, hdm[0], color = "black")  # 画出第一段数据熵图

    e3 = np.arange(dm[len(dm) - 1], dm[len(dm) - 1] + M, 0.00001)  # M为边距值，用以修补无穷区间，而画出有限区间，这是最后一段的值恒定为1
    h[-1] = (math.log(len(x)) / math.log(len(x)))
    hdm[-1] = h[-1] * np.ones(len(e3))
    plt.plot(e3, hdm[-1], color = "black")  # 画出最后一段数据熵图
